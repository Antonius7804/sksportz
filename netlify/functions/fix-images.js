...
exports.handler = async (event) => {
  ...
  let changes = 0;
  for (const [altText, newUrl] of Object.entries(FIXES)) {
    const escaped = altText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const regex = new RegExp(`(${escaped}[^>]*>|<img[^>]*${escaped}[^>]*)src="[^"]*"`, 'g');
    let newContent = content.replace(regex, (match) => match.replace(/src="[^"]*"/, `src="${newUrl}"`));
    if (newContent === content) {
      const regex2 = new RegExp(`src="[^"]*"([^>]*${escaped})`, 'g');
      newContent = content.replace(regex2, `src="${newUrl}"$1`);
    }
    if (newContent !== content) { changes++; content = newContent; }
  }
  const putData = JSON.stringify({ message: `Fix ${changes} article images`, content: Buffer.from(content).toString('base64'), sha: file.sha });
